...
import { Trophy, TrendingUp, Users, Star, ArrowUp, ArrowDown, Medal, Crown, Award, Target, Filter, Settings, BarChart3 } from 'lucide-react';
import { SessionData } from '@/hooks/useSessionsData';
interface EnhancedClassFormatRankingsProps {
  data: SessionData[];
}
export const EnhancedClassFormatRankings: React.FC<EnhancedClassFormatRankingsProps> = ({
  data
}) => {
  ...
  const rankings = useMemo(() => {
    ...
    if (excludeHosted) {
      filteredData = data.filter(session => !session.sessionName?.toLowerCase().includes('hosted') && !session.sessionName?.toLowerCase().includes('myriad'));
    }
    ...
    const processedStats = Object.values(classStats).filter((stats: any) => stats.totalSessions >= minSessions).map((stats: any) => {
  ...
  const RankingCard = ({
    ...
    index: number;
    isBottom?: boolean;
  }) => <div className={`p-4 rounded-xl border-2 transition-all duration-200 hover:shadow-lg ${!isBottom && index === 0 ? 'border-yellow-200 bg-gradient-to-r from-yellow-50 to-amber-50 shadow-md' : !isBottom && index === 1 ? 'border-gray-200 bg-gradient-to-r from-gray-50 to-slate-50' : !isBottom && index === 2 ? 'border-orange-200 bg-gradient-to-r from-orange-50 to-red-50' : isBottom && index === 0 ? 'border-red-200 bg-gradient-to-r from-red-50 to-pink-50' : 'border-slate-200 bg-white hover:border-slate-300'}`}>
