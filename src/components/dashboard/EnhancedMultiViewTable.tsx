...
import { Table2, Calendar, Users, Target, TrendingUp, Clock, MapPin, BarChart3, ArrowUpDown, TrendingDown, Minus, Filter, ChevronRight } from 'lucide-react';
import { SessionData } from '@/hooks/useSessionsData';
interface EnhancedMultiViewTableProps {
  data: SessionData[];
}
type GroupingOption = 'class' | 'trainer' | 'location' | 'day' | 'time' | 'classTrainer' | 'classLocation' | 'trainerLocation' | 'dayTime' | 'classDay';
type MetricSelector = 'attendance' | 'revenue' | 'fillRate' | 'conversion' | 'retention' | 'cancellation' | 'score';
export const EnhancedMultiViewTable: React.FC<EnhancedMultiViewTableProps> = ({
  data
}) => {
  ...
  const getMetricValue = (item: any) => {
    switch (selectedMetric) {
      case 'attendance':
        return item.avgAttendance;
      case 'revenue':
        return item.avgRevenue;
      case 'fillRate':
  ...
  const columns = [{
    ...
    render: (value: string) => <div className="font-medium text-slate-800 max-w-xs">
    ...
    render: (_: any, item: any) => {
      ...
      return <div className={`font-semibold text-${metricConfig?.color}-600`}>
    ...
    header: 'Score',
    align: 'center' as const,
    render: (value: number) => <div className={`font-bold ${value >= 80 ? 'text-yellow-600' : value >= 60 ? 'text-blue-600' : 'text-slate-500'}`}>
          {value}
        </div>
  }];
